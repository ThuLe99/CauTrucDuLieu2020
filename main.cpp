...
void menuTONG(listMH &lmh,LISTLOP &dsl,TREE &t ,IDNGAUNHIEN &idngaunhien,LISTCHDT_SV &lchdt_sv )
{	
	LOP l;
	LISTDT ldt;
	CAUHOIMA chm;
...
void DangNhapGiaoVien_SinhVien(listMH &lmh,LISTLOP &dsl,TREE &t,IDNGAUNHIEN &idngaunhien,CAUHOIMA &chm,LISTCTL &lctl,CAUHOINGAUNHIEN &randomquestion,LISTCHDT_SV &lchdt_sv)
{
	while(true)
	{
		int flag ;
		while(true)
		{
			...
			{
				menuTONG(lmh,dsl,t, idngaunhien,lchdt_sv);
				system("pause");
				break;
			}
...
int main(int argc, char** argv) {
	...
	DangNhapGiaoVien_SinhVien(lmh,dsl,t, idngaunhien,chm, lctl, rdqt,lchdt_sv);
	for(int i=0;i<dsl.soluonglop;i++)
	{
